module Commands.Parser (commandParser) where
...
toCommand (ParsedCommandRequest (BulkString rawCmdStr) args) =
    let normalizedCmdStr = BS.map toUpper rawCmdStr
     in case (normalizedCmdStr, args) of
            ("PING", []) -> Right $ Ping Nothing
            ("PING", [BulkString arg]) -> Right . Ping . Just $ arg
            ("PING", _) -> Left "PING command does accept multiple arguments"
            ("ECHO", []) -> Left "ECHO command requires at least 1 argument. None was provided"
            ("ECHO", [BulkString arg]) -> Right . Echo $ arg
            ("ECHO", _) -> Left "ECHO command requires only 1 argument"
            (unimplementedCommandStr, _) -> handleUnimplementedCmdStrConversion unimplementedCommandStr

handleUnimplementedCmdStrConversion :: ByteString -> Either Text Command
