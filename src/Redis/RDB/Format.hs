...
-}
data AuxField = AuxFieldRedisVer RedisVersion | AuxFieldRedisBits RedisBits | AuxFieldCTime CTime | AuxFieldUsedMem UsedMem | AuxFieldCustom RDBString RDBVal
    deriving stock (Show, Eq)
...
instance RDBBinary EOF where
    ...
    rdbGet = do
        ...
        if tag == eofOpCodeByteTag
            then pure EOF
            else fail [i|Expected EOF marker (#{eofOpCodeByteTag}) got #{show tag} instead|]
...
instance RDBBinary KeyValWithExpiryInMS where
    ...
    rdbGet = do
        ...
        if tag /= keyValWithExpiryInMSOpCodeByteTag
            then fail [i|Expected KeyValWithExpiryInMS marker (#{keyValWithExpiryInMSOpCodeByteTag}) got #{show tag} instead|]
...
instance RDBBinary KeyValWithExpiryInS where
    ...
    rdbGet = do
        ...
        if tag /= keyValWithExpiryInSpCodeByteTag
            then fail [i|Expected KeyValWithExpiryInS marker (#{keyValWithExpiryInSpCodeByteTag}) got #{show tag} instead|]
...
instance RDBBinary AuxField where
    ...
    rdbGet = do
        ...
        if tag /= auxiliaryFieldOpCodeByteTag
            then fail [i|Expected AuxField marker (#{auxiliaryFieldOpCodeByteTag}) got #{show tag} instead|]
            else do
                asum
                    ...
                    , AuxFieldCustom <$> rdbGet @RDBString <*> rdbGet @RDBVal
...
instance RDBBinary RedisBits where
    rdbPut x = do
        ...
        rdbPut (RDBShortString "redis-bits")
        case x of
            RedisBits32 -> rdbPut (toRDBVal "32")
            RedisBits64 -> rdbPut (toRDBVal "64")
    rdbGet = do
        -- Parse and validate "redis-bits" key
        key <- rdbGet @RDBShortString -- "redis-bits" is 10 bytes
        if key /= RDBShortString "redis-bits"
            then fail [i|Expected redis-bits marker "redis-bits" got #{show key} instead|]
            else do
                -- Parse architecture value ("32" or "64")
                tag <- rdbGet @RDBVal
                case fromRDBVal tag of
                    "32" -> pure RedisBits32
                    "64" -> pure RedisBits64
...
instance RDBBinary RedisVersion where
    rdbPut (RedisVersion version) = do
        ...
        rdbPut (RDBShortString "redis-ver")
        rdbPut version
    rdbGet = do
        -- Parse and validate "redis-ver" key
        key <- rdbGet @RDBShortString -- "redis-ver" is 10 bytes
        if key /= RDBShortString "redis-ver"
            then fail [i||Expected redis-ver marker "redis-ver" got #{show key} instead|]
            else do
...
instance RDBBinary CTime where
    rdbPut (CTime cTime) = do
        ...
        rdbPut (toRDBVal (BSC.pack . show $ cTime.getRDBUnixTimestampS))

    rdbGet = do
        -- Parse and validate "ctime" key
        key <- rdbGet @RDBShortString -- "ctime" is 5 bytes
        if key /= RDBShortString "ctime"
            then fail [i|Expected ctime marker "ctime" got #{show key} instead|]
            else do
                -- Parse timestamp string and convert to numeric value
                timeStr <- rdbGet @RDBVal
                let cTimeM = BSC.readWord32 . fromRDBVal $ timeStr
...
instance RDBBinary UsedMem where
    rdbPut (UsedMem mem) = do
        ...
        rdbPut (RDBShortString "used-mem")
        rdbPut (toRDBVal (BSC.pack . show $ mem))

    rdbGet = do
        -- Parse and validate "used-mem" key
        key <- rdbGet @RDBShortString -- "used-mem" is 8
        if key /= RDBShortString "used-mem"
            then fail [i|Expected used-mem marker "used-mem" got #{show key} instead|]
            else do
                -- Parse memory value string and convert to integer
                memVal <- fromRDBVal <$> rdbGet @RDBVal
...
instance RDBBinary SelectDB where
    rdbPut (SelectDB dbIndex) = do
        ...
        rdbPut (RDBLengthPrefix6 dbIndex)
    rdbGet = do
        -- Read and validate database selection opcode
        tag <- genericRDBGet @Word8
        if tag /= selectDBOpCodeByteTag
            then fail [i|Expected SelectDB marker (#{selectDBOpCodeByteTag}) got #{show tag} instead|]
            else do
                -- Parse database index from variable-length encoding
                dbIndex <- rdbGet @RDBLengthPrefix
                pure $ SelectDB (fromRDBLengthPrefix dbIndex)
...
instance RDBBinary ResizeDB where
    rdbPut (ResizeDB numOfKeys numOfKeysWithExpiry) = do
        ...
        rdbPut (RDBLengthPrefix6 numOfKeys)
        rdbPut (RDBLengthPrefix6 numOfKeysWithExpiry)

    rdbGet = do
        tag <- genericRDBGet @Word8
        if tag /= resizeDBOpCodeByteTag
            then fail [i|Expected ResizeDB marker (#{resizeDBOpCodeByteTag}) got #{show tag} instead|]
            else do
                numOfKeys <- rdbGet @RDBLengthPrefix
                numOfKeysWithExpiry <- rdbGet @RDBLengthPrefix
                pure $ ResizeDB (fromRDBLengthPrefix numOfKeys) (fromRDBLengthPrefix numOfKeysWithExpiry)
