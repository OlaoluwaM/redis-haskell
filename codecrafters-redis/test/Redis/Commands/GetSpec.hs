    ...
    beforeAll initializeServerState $ do
        describe "GET Command Handler Tests" $ do
            it "should return NULL for non-existent key" $ \_ -> do
                ...
                result <- runTestServer (handleCommandReq @ServerContext cmdReq) (PassableTestContext{config = Nothing, serverState = Nothing, metadata = Nothing})
            ...
            it "should retrieve a previously stored string value" $ \initialServerState -> do
                ...
                runTestServer (handleCommandReq @ServerContext setCmdReq) (PassableTestContext{config = Nothing, serverState = Just initialServerState, metadata = Nothing})

                result <- runTestServer (handleCommandReq @ServerContext getCmdReq) (PassableTestContext{config = Nothing, serverState = Just initialServerState, metadata = Nothing})
            ...
            it "should return an error for non-string value types" $ \initialServerState -> do
                let key = "car:1"
                let cmdReq = mkCmdReqStr [getCmd, mkBulkString key]
                result <- runTestServer (handleCommandReq @ServerContext cmdReq) (PassableTestContext{config = Nothing, serverState = Just initialServerState, metadata = Nothing})
            ...
            context "Supports key expiry" $ do
                ...
                it "should return value for a key with a TTL that has not expired" $ \serverState -> do
                    ...
                    runTestServer (handleCommandReq @ServerContext setCmdReq) (PassableTestContext{config = Nothing, serverState = Just serverState, metadata = Nothing})

                    let getCmdReq = mkCmdReqStr [getCmd, mkBulkString key]

                    result <- runTestServer (handleCommandReq @ServerContext getCmdReq) (PassableTestContext{config = Nothing, serverState = Just serverState, metadata = Nothing})
